        ...
        match{
            ...
            [&](ArenaVec<Chunk<InitializerMIR>>& aggregate) -> Optional<Chunk<ConstInitLIR>> {
                if (type->is_class()) {
                    ...
                    ECC_UNREACHABLE("invalid type for try_build_constinit_agg");
                }
            },
...
ConstInitLIRBuilder::try_build_constinit_expr(Type *type, CastExprMIR& expr) {
    ...
    ECC_ASSERT(type == expr.target, "type mismatch for building constinit CastExprMIR");
...
ConstInitLIRBuilder::try_build_constinit_expr(Type *type, IdentExprMIR& expr) {
    ...
    if (auto *var = expr.ident->as_varsym(); var) {
        ...
        ECC_ASSERT_N(expr.ident->is_func());
        LIRFuncSym *sym = syms.get().lookup_func(expr.ident->as_funcsym());

        ECC_ASSERT_N(sym);
        ECC_ASSERT_N(sym->lir);

        auto init = make_chunk<FuncInitLIR>(expr.loc, sig, sym->lir);
        return init;

    } else {
        ECC_UNREACHABLE("invalid type for building constinit IdentExprMIR");
    }
}
