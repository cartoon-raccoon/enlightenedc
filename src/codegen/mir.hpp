...
public:
    ProgramItemMIR(Location loc) : MIRNode(loc) {}
...
public:
    ExprMIR(Location loc) : MIRNode(loc) {}
    ExprMIR(Location loc, sema::types::Type *type) : MIRNode(loc), type(type) {}
...
public:
    DeclMIR(Location loc) : ProgramItemMIR(loc) {}
    virtual void accept(MIRVisitor& visitor) = 0;
};

class StmtMIR : public ProgramItemMIR {
public:
    StmtMIR(Location loc) : ProgramItemMIR(loc) {}
    virtual void accept(MIRVisitor& visitor) = 0;
};
...
public:
    InitializerMIR(Location loc, Box<ExprMIR> expr)
        : MIRNode(loc), initializer(std::move(expr)) {}
    InitializerMIR(Location loc, Vec<Box<InitializerMIR>> initializers)
        : MIRNode(loc), initializer(std::move(initializers)) {}
...
public:
    ...
    VarDeclMIR(Location loc, sema::sym::VarSymbol *sym, Box<InitializerMIR> init)
        : DeclMIR(loc), sym(sym), initializer(std::move(init)) {}
...
public:
    ...
    CompoundStmtMIR(Location loc, Vec<Box<ProgramItemMIR>> items)
        : StmtMIR(loc), items(std::move(items)) {}
...
public:
    ExprStmtMIR(Location loc, Box<ExprMIR> expr)
        : StmtMIR(loc), expr(std::move(expr)) {}
...
public:
    SwitchStmtMIR(Location loc, Box<ExprMIR> condition, Box<StmtMIR> body)
        : StmtMIR(loc), condition(std::move(condition)), body(std::move(body)) {}
...
public:
    DefaultStmtMIR(Location loc, Box<StmtMIR> stmt)
        : StmtMIR(loc), stmt(std::move(stmt)) {}
...
public:
    LabeledStmtMIR(Location loc, sema::sym::LabelSymbol *label, Box<StmtMIR> stmt)
        : StmtMIR(loc), label(label), stmt(std::move(stmt)) {}
...
public:
    PrintStmtMIR(Location loc, std::string format_string, Vec<Box<ExprMIR>> arguments)
        : StmtMIR(loc), format_string(format_string), arguments(std::move(arguments)) {}
...
public:
    BreakStmtMIR(Location loc) : StmtMIR(loc) {}
...
public:
    ReturnStmtMIR(Location loc, Box<ExprMIR> ret_expr)
        : StmtMIR(loc), ret_expr(std::move(ret_expr)) {}
...
public:
    UnaryExprMIR(Location loc, Box<ExprMIR> operand, tokens::UnaryOp op)
        : ExprMIR(loc), operand(std::move(operand)), op(op) {}
...
public:
    CastExprMIR(Location loc, sema::types::Type *target, Box<ExprMIR> inner)
        : ExprMIR(loc), target(target), inner(std::move(inner)) {}
...
public:
    CondExprMIR(Location loc, Box<ExprMIR> condition,
        ...
        : ExprMIR(loc), true_expr(std::move(true_expr)),
...
public:
    LiteralExprMIR(Location loc, Box<exec::Value> value)
        : ExprMIR(loc), value(std::move(value)) {}
...
public:
    CallExprMIR(Location loc, Box<ExprMIR> callee, Vec<Box<ExprMIR>> args)
        : ExprMIR(loc), callee(std::move(callee)), args(std::move(args)) {}
...
public:
    MemberAccExprMIR(Location loc, Box<ExprMIR> object, std::string member)
        : ExprMIR(loc), object(std::move(object)), member(member) {}
...
public:
    ProgramMIR(Location loc) : MIRNode(loc), items() {}

    ProgramMIR(Location loc, Vec<Box<ProgramItemMIR>> items)
        : MIRNode(loc), items(std::move(items)) {}
